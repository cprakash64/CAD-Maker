  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5 py-8">
      ...
      <div className={`card space-y-3 p-4 ${imageBlocked ? "opacity-50" : ""}`}>
        ...
        <button className="btn-primary" disabled={!file || busy || imageBlocked} onClick={upload}>
          {busy && !interp ? "Reading drawing…" : "Interpret drawing"}
        </button>
      </div>
        ...
        <div className="card space-y-3 p-4">
          ...
          {interp.clarification_questions.length > 0 && (
            <div className="rounded-md border border-amber-500/40 bg-amber-500/10 p-2 text-xs text-amber-100">
              ...
              <p className="mt-2">
                Describe the part in the “Correct interpretation” box above and
                interpret again.
              </p>
            </div>
          )}
