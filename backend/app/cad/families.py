...
_FAMILIES: tuple[CADFamily, ...] = (
    ...
    # ---- Everyday concept-fallback families (single connected solids) ---
    # Common objects a casual user types. Each builds ONE connected concept solid
    # from overlapping primitives (documented defaults in
    # app.cad.standards.defaults.CONCEPT_DEFAULTS) so a vague everyday prompt yields
    # safe, labelled concept CAD instead of broken free-form geometry.
    CADFamily(
        family_id="hammer",
        ...
        object_types=("hammer",),
        required_dimensions=(),
        optional_dimensions=("handle length", "handle diameter", "head size"),
        default_assumptions=("Cylindrical handle with a rectangular head fused on top",
                             "Default ~280mm handle, ~110mm head"),
        generator="hammer concept (deterministic plan)",
        generation_strategy=GenerationStrategy.cadplan,
        validation_profile="single_part_relaxed",
        export_policy=EXPORT_PART,
        known_limitations=(
            "Concept tool — not manufacturing-certified; no claw, wedge, or "
            "ergonomic grip detail.",
        ),
        example_prompts=("Make a hammer", "A hammer with a 300mm handle"),
    ),
    CADFamily(
        family_id="wrench",
        ...
        object_types=("wrench",),
        required_dimensions=(),
        optional_dimensions=("overall length", "head diameter", "jaw opening"),
        default_assumptions=("Flat handle bar with a ring head and a jaw opening",
                             "Default ~200mm long"),
        generator="wrench concept (deterministic plan)",
        generation_strategy=GenerationStrategy.cadplan,
        validation_profile="single_part_relaxed",
        export_policy=EXPORT_PART,
        known_limitations=(
            "Concept tool — a ring-end representation, not a sized/standard "
            "open-end or socket wrench.",
        ),
        example_prompts=("Make a wrench", "A 250mm wrench"),
    ),
    CADFamily(
        family_id="pliers",
        ...
        object_types=("pliers",),
        required_dimensions=(),
        optional_dimensions=("handle length", "jaw length"),
        default_assumptions=("Two jaws and two handles fused at a central pivot",
                             "Modeled in the open position as one connected body"),
        generator="pliers concept (deterministic plan)",
        generation_strategy=GenerationStrategy.cadplan,
        validation_profile="single_part_relaxed",
        export_policy=EXPORT_PART,
        known_limitations=(
            "Concept tool — a single fused body, not an articulated two-part "
            "linkage; no cutting edges or serrations.",
        ),
        example_prompts=("Make pliers", "A pair of pliers"),
    ),
    CADFamily(
        family_id="wheel",
        ...
        object_types=("wheel",),
        required_dimensions=(),
        optional_dimensions=("diameter", "thickness", "bore"),
        default_assumptions=("Solid disc with a raised hub, axle bore and lightening holes",
                             "Default Ø200mm"),
        generator="wheel concept (deterministic plan)",
        generation_strategy=GenerationStrategy.cadplan,
        validation_profile="single_part_relaxed",
        export_policy=EXPORT_PART,
        known_limitations=(
            "Concept disc wheel — no tyre profile, rim flange, or spoke "
            "engineering.",
        ),
        example_prompts=("Make a wheel", "A 250mm wheel with a 30mm bore"),
    ),
    CADFamily(
        family_id="fan_blade",
        ...
        required_dimensions=(),
        optional_dimensions=("blade length", "hub diameter", "bore"),
        default_assumptions=("Central hub with four flat blades fused around it",
                             "Default 4 blades, Ø44mm hub"),
        generator="fan_blade concept (deterministic plan)",
        generation_strategy=GenerationStrategy.cadplan,
        validation_profile="single_part_relaxed",
        export_policy=EXPORT_PART,
        known_limitations=(
            "Concept fan — flat blades with no aerodynamic twist, camber, or "
            "balance; not a performance impeller.",
        ),
        example_prompts=("Make a fan blade", "A 4-blade fan"),
    ),
    CADFamily(
        family_id="hook",
        ...
        object_types=("hook",),
        required_dimensions=(),
        optional_dimensions=("arm length", "plate size", "mounting holes"),
        default_assumptions=("Wall mounting plate with an arm and an upturned tip",
                             "Two mounting holes by default"),
        generator="hook concept (deterministic plan)",
        generation_strategy=GenerationStrategy.cadplan,
        validation_profile="single_part_relaxed",
        export_policy=EXPORT_PART,
        known_limitations=(
            "Concept hook — blocky right-angle arm, not a smooth curved or "
            "load-rated hook.",
        ),
        example_prompts=("Make a hook", "A wall hook with two mounting holes"),
    ),
    CADFamily(
        ...
        required_dimensions=(),
        optional_dimensions=("grip length", "standoff height", "mounting holes"),
        default_assumptions=("Grip bar on two standoffs with mounting holes",
                             "Default 160mm grip"),
        generator="generic_handle concept (deterministic plan)",
        generation_strategy=GenerationStrategy.cadplan,
        validation_profile="single_part_relaxed",
        export_policy=EXPORT_PART,
        known_limitations=(
            "Concept pull handle — straight grip on two posts; not an ergonomic "
            "or revolved-profile handle.",
        ),
        example_prompts=("Make a handle", "A drawer pull handle 160mm long"),
    ),
    CADFamily(
        ...
        required_dimensions=(),
        optional_dimensions=("width", "depth", "tool hole size", "tool hole count"),
        default_assumptions=("L-profile base + back wall with a row of tool holes",
                             "Default 200×60mm with 4 holes"),
        generator="tool_holder concept (deterministic plan)",
        generation_strategy=GenerationStrategy.cadplan,
        validation_profile="single_part_relaxed",
        export_policy=EXPORT_PART,
        known_limitations=(
            "Concept holder — plain round tool holes, no contoured pockets or "
            "labels.",
        ),
        example_prompts=("Make a tool holder", "A tool holder with 5 holes"),
    ),
    CADFamily(
        ...
        object_types=("stand",),
        required_dimensions=(),
        optional_dimensions=("top size", "height"),
        default_assumptions=("Flat top platform on four legs",
                             "Default 160×120mm top, 120mm tall"),
        generator="generic_stand concept (deterministic plan)",
        generation_strategy=GenerationStrategy.cadplan,
        validation_profile="single_part_relaxed",
        export_policy=EXPORT_PART,
        known_limitations=(
            "Concept stand — four straight legs and a flat top; not angle- or "
            "load-optimized.",
        ),
        example_prompts=("Make a stand", "A 200mm tall stand"),
    ),
    CADFamily(
        ...
        object_types=("casing",),
        required_dimensions=(),
        optional_dimensions=("width", "depth", "height", "wall thickness"),
        default_assumptions=("Shelled box with an open top",
                             "Default 120×80×50mm, 3mm walls"),
        generator="simple_casing concept (deterministic plan)",
        generation_strategy=GenerationStrategy.cadplan,
        validation_profile="single_part_relaxed",
        export_policy=EXPORT_PART,
        known_limitations=(
            "Concept casing — a single open-top shell; no lid, bosses, ribs, or "
            "sealing features (see the enclosure family for those).",
        ),
        example_prompts=("Make a simple casing", "A casing 100x60x40mm"),
    ),
