...
# Harmless for SQLite. pool_size/max_overflow/pool_timeout are explicit
# (docs/ops/observability.md: "DB connection-pool protection") rather than
# left at SQLAlchemy's silent defaults, so a burst of concurrent requests
# fails fast with a clear pool-timeout error instead of piling up unbounded
# connections against Postgres -- SQLite (single-file, no real pool) ignores
# these, matching the DB-specific kwargs pattern already used for pragmas.
